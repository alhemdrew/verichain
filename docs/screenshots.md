...
| [07-verification-screen.png](screenshots/07-verification-screen.png) | Presented-file verification screen. A result is shown only after a file comparison completes. |
...
The complete local capture set is stored in `~/VeriChain-Screenshots`, organized by workflow area. It includes the login, dashboard, cases, evidence, verification, sharing, and report routes.
